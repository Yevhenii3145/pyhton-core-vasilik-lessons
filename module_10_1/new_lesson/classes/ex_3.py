...
for el in (cat, dog):
    if type(el) is Dog:
        print(f"{el.sound()} | {el.get_info()}") # Rex says Woof | It's dog. His name: Rex and he's 3 years old

for el in (cat, dog):
    if type(el) is Cat:
        print(f"{el.sound()} | {el.get_info()}") # Boris says Meow | It's cat. His name: Boris and he's 4 years old
